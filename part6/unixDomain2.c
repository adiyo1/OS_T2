...
int main(int argc, char *argv[]) {
    ...
    if (flagb && input_spec != NULL && output_spec != NULL && strcmp(input_spec, output_spec) == 0) {
        if (strncmp(input_spec, "TCPS", 4) == 0) {
            ...
            out_fd = in_fd;
        } else if (strncmp(input_spec, "UDSSD", 6) == 0) {
            in_fd = start_uds_server_datagram(input_spec + 6);
            out_fd = in_fd;
        } else if (strncmp(input_spec, "UDSSS", 6) == 0) {
            int server_fd = start_uds_server_stream(input_spec + 6);
